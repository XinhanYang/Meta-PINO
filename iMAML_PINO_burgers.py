...
class InnerNet(
    ...
    def objective(self, x, y):
        out = self(x).reshape(y.shape)

        loss_l2 = self.loss_fn(out, y)

        loss_ic, loss_f = PINO_loss(out, x[:, 0, :, 0], self.v)
    ...
    for ep in pbar:
        ...
        for x_batch, y_batch in train_loader:
            ...
            for i in range(batch_size):
                ...
                out_instance = optimal_inner_net(x_instance).reshape(y_instance.shape)
